<#
  ...
  Pass -UpdateVercel to also push the fresh public tunnel URL to Vercel and redeploy
  production (requires the Vercel CLI logged in + linked once - see tunnel-url.ps1):

      powershell -ExecutionPolicy Bypass -File .\up.ps1 -UpdateVercel
#>
param(
    [switch]$UpdateVercel
)
...
# -- 0. Preflight: Docker Desktop must be running ----------------------------
# minikube's docker driver talks to the Docker daemon; if Docker Desktop is not
# up, `minikube start` fails with a cryptic named-pipe error and the rest of the
# script cascades. Fail fast with a clear message instead.
Step "Checking Docker is running"
# Judge Docker purely by exit code. docker can write harmless warnings (e.g. the
# seccomp note) to stderr; under $ErrorActionPreference='Stop' a redirected stderr
# line is wrapped as a TERMINATING error in PS 5.1, so we drop to 'Continue' just
# for this probe and discard all streams.
$ErrorActionPreference = "Continue"
docker info *> $null
$dockerOk = ($LASTEXITCODE -eq 0)
$ErrorActionPreference = "Stop"
if (-not $dockerOk) {
    throw "Docker is not reachable. Start Docker Desktop, wait until it reports 'running', then re-run .\up.ps1"
}
...
# --unset emits Remove-Item for vars that may not exist on this machine (e.g.
# SSH_AUTH_SOCK); under 'Stop' that would halt us, so soften just this step.
$ErrorActionPreference = "Continue"
...
# Optionally push the fresh URL to Vercel + redeploy production. VITE_* vars are
# baked at build time, so a redeploy is required for the new URL to take effect.
if ($UpdateVercel -and $tunnel) {
    Step "Updating VITE_API_URL on Vercel + redeploying production"
    $frontend = Join-Path $PSScriptRoot "frontend"
    Push-Location $frontend
    try {
        try { vercel env rm VITE_API_URL production --yes 2>$null } catch {}
        $tunnel | vercel env add VITE_API_URL production
        vercel --prod
        Write-Host "`n  Vercel redeployed against $tunnel" -ForegroundColor Green
    }
    catch {
        Write-Host "  Vercel update failed: $($_.Exception.Message)" -ForegroundColor Yellow
        Write-Host "  Is the Vercel CLI installed + linked? (npm i -g vercel; vercel login; cd frontend; vercel link)" -ForegroundColor Yellow
    }
